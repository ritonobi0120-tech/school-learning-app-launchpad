(function () {
  ...
  const els = {
    ...
    bonusStageSelect: document.getElementById('bonusStageSelect'),
  ...
  const STAGE_CARD_IMAGE_PATHS = {
    ...
    'extra-practice': 'assets/generated/stage-card-extra-practice.png',
    'oni-challenge': 'assets/generated/stage-card-oni-challenge.png',
  };
  ...
  function renderQuestion() {
    ...
    els.comboChip.textContent = `あと${Math.max(0, getCurrentGoal() - stageKeys)}問`;
    ...
    return `
      <div class="simple-clear-panel">
        ...
        <div class="simple-clear-center">
          <img src="${artifactIcon(stage.id)}" alt="">
          <span class="simple-progress-label">${label}</span>
          <strong>${stageKeys}/${goal}</strong>
  ...
  function isBonusModeAvailable(mode) {
    if (mode === 'extra') return isAllClear();
    if (mode === 'oni') return isExtraClear();
    return false;
  }

  function getSelectedBonusMode() {
    return isBonusModeAvailable(selectedBonusMode) ? selectedBonusMode : null;
  }
  ...
  function getCurrentPathMark(stageId) {
    ...
    return Math.min(getCurrentGoal(), session.startKeys + session.index + 1);
  }

  function getProjectedStagePathCount(stageId, result = null) {
    if (!session || session.reviewOnly || session.stageId !== stageId) return getProjectedStageKeyCount(stageId);
    const answeredStep = session.answered && result && result.correct ? 1 : 0;
    return Math.min(getCurrentGoal(), session.startKeys + session.index + answeredStep);
  }
  ...
  function ensureSelectedStageUnlocked() {
    const pendingStageId = getPendingReviewStageId();
    if (pendingStageId) {
      selectedBonusMode = null;
      selectedStageId = pendingStageId;
      return;
    }
    const saved = loadActiveSession();
    if (!homeStageManuallySelected && saved && saved.oniMode && isBonusModeAvailable('oni')) {
      selectedBonusMode = 'oni';
      selectedStageId = EXTRA_STAGE_ID;
      return;
    }
    if (!homeStageManuallySelected && saved && saved.extraMode && isBonusModeAvailable('extra')) {
      selectedBonusMode = 'extra';
      selectedStageId = EXTRA_STAGE_ID;
      return;
    }
    ...
    if (selectedBonusMode && !isBonusModeAvailable(selectedBonusMode)) selectedBonusMode = null;
    if (!homeStageManuallySelected && !selectedBonusMode) {
      if (isBonusModeAvailable('oni')) {
        selectedBonusMode = 'oni';
        selectedStageId = EXTRA_STAGE_ID;
        return;
      }
      if (isBonusModeAvailable('extra')) {
        selectedBonusMode = 'extra';
        selectedStageId = EXTRA_STAGE_ID;
        return;
      }
  ...
  function selectHomeBonusMode(mode, { sound = false } = {}) {
    const pendingStageId = getPendingReviewStageId();
    if (pendingStageId || !isBonusModeAvailable(mode)) return false;
    homeStageManuallySelected = true;
    selectedStageId = EXTRA_STAGE_ID;
    if (selectedBonusMode === mode) return true;
    if (sound) playSound('tap');
    selectedBonusMode = mode;
    renderStageSelect();
    renderHomeStats();
    return true;
  }
  ...
  function startHomeSelection() {
    ...
    const bonusMode = getSelectedBonusMode();
    if (bonusMode === 'oni') {
      startOniSession();
      return;
    }
  ...
  function renderHomeStats() {
    ...
    if (els.homeGoal) {
      els.homeGoal.textContent = hasMistakes
        ? '今の目的：見直しで続きの道をひらこう'
        : bonusMode === 'oni'
          ? `鬼30問 ${oniKeys}/${ONI_GOAL}`
        : bonusMode === 'extra'
          ? `おかわり30問 ${extraKeys}/${EXTRA_GOAL}`
        : oniClear
    ...
    els.startButton.textContent = hasMistakes
      ...
      : savedSession && savedSession.stageId === selectedStageId
        ? 'つづきから'
        : bonusMode === 'oni' && oniClear
          ? '鬼30問 もう一度'
        : bonusMode === 'oni'
          ? '鬼30問へ'
        : bonusMode === 'extra' && extraClear
          ? 'おかわり30問 もう一度'
        : bonusMode === 'extra'
          ? 'おかわり30問へ'
        : isStageCleared(selectedStageId)
          ...
          : 'はじめる';
    if (els.extraButton) {
      els.extraButton.classList.add('hidden');
      els.extraButton.textContent = '';
    }
  ...
  function renderSessionMap(stageId, keyCount, pathCount, pulse, missPulse, markState = {}) {
    ...
    const heroStep = path > 0 && path % SESSION_LENGTH === 0
      ? path
      : Math.min(goal, Math.max(1, path + 1));
    ...
    const windowEnd = Math.min(goal, windowStart + SESSION_LENGTH - 1);
  ...
  function renderStageSelect() {
    ...
    els.stageSelect.innerHTML = core.STAGES.map((stage) => {
      const active = !bonusMode && stage.id === selectedStageId ? 'active' : '';
      ...
      return `
        <button class="stage-card ${active} ${stateClass}" type="button" data-stage="${stage.id}" data-state-label="${stateLabel}" aria-pressed="${!bonusMode && stage.id === selectedStageId ? 'true' : 'false'}" ${unlocked && !blockedByReview ? '' : 'disabled'}>
    ...
    renderBonusStageSelect();
  }

  function bonusModeCards() {
    const extraKeys = getExtraKeyCount();
    const oniKeys = getOniKeyCount();
    const cards = [];
    if (isAllClear()) {
      cards.push({
        ...
        title: 'おかわり',
        sub: '追加30問',
        count: `${extraKeys}/${EXTRA_GOAL}`,
        icon: stageCardBadge('extra-practice'),
        status: extraKeys >= EXTRA_GOAL ? 'クリア済み' : '選んで はじめる',
      });
    }
    if (isExtraClear()) {
      cards.push({
        ...
        title: '鬼',
        sub: 'ラスト30問',
        count: `${oniKeys}/${ONI_GOAL}`,
        icon: stageCardBadge('oni-challenge'),
        status: oniKeys >= ONI_GOAL ? '超完全クリア' : '選んで はじめる',
      });
    }
    return cards;
  }

  function renderBonusStageSelect() {
    if (!els.bonusStageSelect) return;
    const pendingStageId = getPendingReviewStageId();
    const cards = pendingStageId ? [] : bonusModeCards();
    els.bonusStageSelect.classList.toggle('hidden', !cards.length);
    els.bonusStageSelect.innerHTML = cards.map((card) => {
      const active = getSelectedBonusMode() === card.mode ? 'active' : '';
      const parts = card.count.split('/').map((value) => Math.max(0, Number(value) || 0));
      const progressPct = `${Math.round((parts[0] / Math.max(1, parts[1])) * 100)}%`;
      return `
        <button class="bonus-card ${card.typeClass} ${active}" type="button" data-bonus-mode="${card.mode}" aria-pressed="${active ? 'true' : 'false'}">
          <span class="stage-select-pencil" aria-hidden="true"></span>
          <b class="stage-order" aria-hidden="true">${card.number}</b>
          <img class="stage-badge" src="${card.icon}" alt="">
          <span>${card.label}</span>
          <strong>${card.title}</strong>
          <small>${card.sub}</small>
          <span class="stage-count-pill"><b>${card.count}</b></span>
          <i class="stage-progress" style="--pct:${progressPct}" aria-hidden="true"></i>
          <em>${card.status}</em>
        </button>
      `;
    }).join('');
    els.bonusStageSelect.querySelectorAll('[data-bonus-mode]').forEach((button) => {
      const mode = button.dataset.bonusMode;
      button.addEventListener('pointerenter', () => {
        selectHomeBonusMode(mode);
      });
      button.addEventListener('focus', () => {
        selectHomeBonusMode(mode);
      });
      button.addEventListener('click', () => {
        unlockAudio();
        selectHomeBonusMode(mode, { sound: true });
      });
    });
  }
