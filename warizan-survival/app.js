...
function renderHome() {
  ...
  const completeBadge = state.best >= CLEAR_COUNT
    ? `
        <div class="home-complete-badge" aria-label="100問コンプリート">
          <span>★</span>
      ...
      <div class="home-screen">
        ...
        <div class="home-bottom">
          <div class="home-mission">
            <strong>わり算で生き残れ</strong>
            <span>現在の最高記録 <b>${state.best}</b>問</span>
          </div>
          <button class="primary home-cta" type="button" data-action="start">挑戦する</button>
        </div>
      </div>
    </section>
  `;
...
function renderGame() {
  ...
  app.innerHTML = `
    <section class="screen">
      <div class="game-card session-card compact-session">
        <div class="session-topbar">
          ...
          <div class="session-topbar-right">
            <span class="session-best">サバイバル ${state.answered}問</span>
...
function updateTimerDisplay(remaining) {
  ...
  if (gameCard) gameCard.classList.toggle("fx-hurry", remaining <= 3);
}
    ...
    feedback.innerHTML = `
      <div class="answer-reveal" role="alert">
        ...
        <div class="wrong-result-actions">
          <button class="primary reveal-retry" type="button" data-action="start">もう一回挑戦</button>
...
function finishGame(reason, userAnswer) {
  cancelAnimationFrame(state.rafId);
  clearTimeout(state.nextTimerId);
  ...
  app.innerHTML = `
    <section class="screen result-shell">
      <div class="result-card ${resultClass}">
        ...
        <div class="actions">
          <button class="primary" type="button" data-action="start">もう一回挑戦</button>
    ...
    <section class="screen howto-screen">
      <div class="result-card howto-card">
        ...
        <div class="review howto-panel">
          ...
          <span>「挑戦する」を押すと、すぐに第1問が始まる。</span>
